<template>
    <div class="body">
        ...
        <div id="titleBar">
            <span class="name">
                <span>
                    <span style="color: RGB(87,96,113); size: 25px; "> Welcome back,admin </span>
                </span>
            </span>
            <ul>
                <li style="cursor: default" @click="exit">退出登录</li>
                <li style="cursor: default;" @click="sendmessage">上传视频</li>
            </ul>
        </div>
        ...
        <!-- <div id="selecctor">
            <Selector_vedio v-for="(play_video, index) in playlist" :vedio_item=play_video @click="playVideo(index)"></Selector_vedio>
        </div> -->
        <!--播放列表-->
        <div id="selector">
            <div id="Per" v-for="(play_video, index) in playlist">
                <button @click="deleteVideo(index)">删除</button>
                <div id="title" @click="playVideo(index)">{{ play_video.videoName }}</div>
                <div id="descript" @click="playVideo(index)">{{ play_video.videoDescript }}</div>
            </div>
        </div>

        <!--信息输入-->
        <div class="loginbox" v-show="getmessage">
            <!-- 左侧的注册盒子 -->
            <div class="loginbox-in">
                <div class="userbox">
                    <span class="iconfont">&#xe817;</span>
                    <input class="user" id="user" v-model="name" placeholder="视频名">
                </div>
                <br>
                <div class="pwdbox">
                    <span class="iconfont">&#xe817;</span>
                    <input class="pwd" id="password" v-model="url" placeholder="视频地址">
                </div>
                <br>
                <div class="pwdbox">
                    <span class="iconfont">&#xe817;</span>
                    <input class="pwd" id="re_password" v-model="descript"  placeholder="视频内容描述">
                </div>

                <br>
                <button type="primary" class="register_btn" @click="uploadVideo">确认</button>
                <button type="primary" class="register_btn" @click="cancle">取消</button>
            </div>

            <!-- 右侧的注册盒子 -->
            <div class="background">
                <div class="title">在这里上传新的视频文件</div>
            </div>
...
export default {
    ...
    methods: {
        ...
        async deleteVideo(index) {
            const res = await request.get(`/delete_vedio/${this.playlist[index].videoId}`)
        ...
        async uploadVideo() {
            ...
            // video.url = this.url
            // video.descript = this.descript
            //连后端
            const res = await request.post('/add_vedio', {
                videoName: this.name,
                videoUrl: this.url,
                videoDescript:this.descript
            })
            this.getmessage = false;
            location.reload()
        },
